...
export default function TrialComponent({ trialCode, trialError, onTrialSuccess }) {
  ...
  const fetchTrialData = async (code) => {
    try {
      ...
      if (data.success) {
        ...
        setTrialData({ error: data.error || '체험 정보를 가져올 수 없습니다.' });
      }
    } catch (error) {
  ...
  if (!trialData) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="text-center py-12">
          <h2 className="text-xl font-bold text-gray-900 mb-2">체험 생성 실패</h2>
          <p className="text-gray-500 text-sm mb-4">체험 코드 생성에 실패했습니다.</p>
          <button
    ...
    return (
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center mb-4">
          ...
          <h2 className="text-xl font-bold text-white mb-1">무료 체험 완료되었습니다.</h2>
          <p className="text-white font-semibold mb-2">지금 당장 특가로 만나보세요!</p>
          <p className="text-white text-sm opacity-80">무료 체험은 계정당 한번만 가능합니다</p>
        </div>
        ...
        <div className="p-1 bg-gradient-to-r from-purple-400 to-blue-400 rounded-2xl">
          ...
          <div className="mb-6">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-lg text-gray-400 line-through">₩550,000</span>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white py-4 px-6 rounded-xl font-semibold text-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
          >
            지금 구매하기
          </button>
  ...
  return (
    <div className="rounded-xl shadow-sm border border-gray-200 p-6" style={{backgroundColor: "#FEFEFE"}}>
      <div className="text-center mb-6">
        <div className="w-32 mx-auto mb-4" style={{
          height: '126px',
          overflow: 'hidden',
          position: 'relative'
        }}>
